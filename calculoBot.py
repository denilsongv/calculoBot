...
CAMINHO_JSON = r"C:\Users\denil\Desktop\programas Python\Novo Clauculo Bot\calculobot-6869566ca9c0.json"
...
def garantir_aba(planilha, nome_aba, colunas):
    try:
        ...
        aba = planilha.add_worksheet(title=nome_aba, rows=1000, cols=max(20, len(colunas)))
...
def salvar_em_aba(planilha, nome_aba, dados, colunas):
    try:
        ...
        # monta a linha nova na ordem das colunas
...
if not df_orc.empty:
    ...
    if filtro_revendedor.strip():
        df_filtrado = df_filtrado[
            df_filtrado["nome_revendedor"].astype(str).str.contains(filtro_revendedor, case=False, na=False)
        ]

    if filtro_cliente.strip():
        df_filtrado = df_filtrado[
            df_filtrado["nome_cliente"].astype(str).str.contains(filtro_cliente, case=False, na=False)
        ]
